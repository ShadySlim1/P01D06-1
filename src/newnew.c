...
int main(void) {
    ...
    char movement;       // команды для передвижения ракеток
    int BallPhase = -1;  // вектор мяча
    print_playing_field(0, y_racket_left, 79, y_racket_rigth, x_ball, y_ball, score_racket_left,
                        score_racket_right);

    while (score_racket_left < 20 && score_racket_right < 20) {
        while (1) {
            scanf("%c", &movement);
            if (movement == 'z' || movement == 'a' || movement == ' ' || movement == 'k' || movement == 'm')
                break;
        }

        if (racket(y_racket_left, movement, 1) == 1 || racket(y_racket_rigth, movement, 2) == 1) {
            if (movement == 'a') {
