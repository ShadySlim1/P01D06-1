...
int main(void) {
    ...
    while (score_racket_left < 21 && score_racket_right < 21) {
        ...
        movement_rigth = movement_left;
        if (racket(y_racket_left, movement_left, 1) == 1 || racket(y_racket_rigth, movement_rigth, 2) == 1) {
            // если мы не вышли за границы то меняем положение ракеток иначе
            // ничего не делаем
            if (movement_left == 'a') {
                y_racket_left = y_racket_left - 1;  // двигаемся вверх
            } else if (movement_left == 'z') {
                y_racket_left = y_racket_left + 1;  // двигаемся вниз
            } else if (movement_rigth == 'k') {
